 ...
 * under the License.
 */
package org.apache.pinot.opal.distributed.keyCoordinator.common;
...
public class DistributedCommonUtils {
  ...
  public static String getKafkaTopicFromTableName(String tableName) {
    return KeyCoordinatorConf.KAFKA_TOPIC_PREFIX + tableName;
  }

  public static String getTableNameFromKafkaTopic(String kafkaTopic) {
    Preconditions.checkState(kafkaTopic.length() > KeyCoordinatorConf.KAFKA_TOPIC_PREFIX.length(), "kafka topic is not valid");
    return kafkaTopic.substring(KeyCoordinatorConf.KAFKA_TOPIC_PREFIX.length());
  }
